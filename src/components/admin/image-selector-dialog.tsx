...
export function ImageSelectorDialog({ isOpen, onOpenChange }: ImageSelectorDialogProps) {
    ...
    useEffect(() => {
        if (isOpen && orgUser) {
            setSelectedImageUrl(orgUser.imageUrl || '');
    ...
    const handleSave = async () => {
        ...
        setIsLoading(true);
        try {
            await updateOrganizationImage(orgUser.id, selectedImageUrl);
            toast({
                title: "Image Updated",
                description: `The logo for ${orgUser.organizationName} has been changed.`,
            });
            onOpenChange(false);
        } catch (error: any) {
             toast({
    ...
    return (
        <Dialog open={isOpen} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-3xl">
                <DialogHeader>
                    <DialogTitle>Change Logo for {orgUser.organizationName}</DialogTitle>
                    <DialogDescription>
